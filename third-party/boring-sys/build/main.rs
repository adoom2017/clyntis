...
#[cfg(windows)]
fn configure_windows_libclang() {
    if let Some(configured) = std::env::var_os("LIBCLANG_PATH") {
        // PowerShell profiles and CI variables sometimes retain the surrounding
        // quotes from `set LIBCLANG_PATH="C:\\..."`. clang-sys treats those
        // quotes as part of the directory name, so normalize them here.
        let normalized = configured.to_string_lossy();
        let normalized = normalized.trim().trim_matches(['"', '\'']);
        let path = PathBuf::from(normalized);
        if path.join("libclang.dll").is_file() {
            // SAFETY: the build script is single-threaded here and bindgen has
            // not initialized libclang yet.
            unsafe { std::env::set_var("LIBCLANG_PATH", &path) };
            println!("cargo:warning=using libclang from {}", path.display());
            return;
        }
        println!(
            "cargo:warning=ignoring invalid LIBCLANG_PATH={}",
            PathBuf::from(configured).display()
        );
    }
    let mut candidates = Vec::new();
    if let Ok(output) = Command::new("where.exe").arg("clang.exe").output() {
    ...
    if let Some(path) = candidates
        .into_iter()
        .find(|path| path.join("libclang.dll").is_file())
    {
        // SAFETY: the build script is single-threaded here and bindgen has not
        // initialized libclang yet.
        unsafe { std::env::set_var("LIBCLANG_PATH", &path) };
        println!("cargo:warning=using libclang from {}", path.display());
    }
}
