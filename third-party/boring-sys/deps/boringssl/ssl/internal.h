...
class SSLKeyShare {
 public:
  ...
  // Copies the X25519 component of a generated key share. This narrowly
  // scoped hook is used by the REALITY client while ClientHello is being
  // serialized. Implementations without an X25519 component return false.
  virtual bool CopyX25519PrivateKey(uint8_t out[32]) const { return false; }

  // Replaces the X25519 component before Generate. Implementations without an
  // X25519 component return false.
  virtual bool SetX25519PrivateKey(const uint8_t in[32]) { return false; }
};
...
struct SSL_CONFIG {
  ...
  // extension_order, when non-empty, contains ClientHello extension type IDs
  // in the exact prefix order requested by the caller. Known extensions not
  // listed here are appended in BoringSSL's table order.
  Array<uint16_t> extension_order;

  // Browser emulation controls which are intentionally configured per
  // connection so cached SSL_CTX objects remain reusable.
  Array<uint16_t> delegated_credential_sigalgs;
  Array<uint16_t> tls13_cipher_order;
  uint16_t record_size_limit = 0;
  bool allow_unadvertised_peer_sigalg = false;
  bool reuse_x25519_key_share = false;
...
struct ssl_ctx_st : public bssl::RefCounted<ssl_ctx_st> {
  ...
  // Client-side, pre-transcript ClientHello mutation hook used by REALITY.
