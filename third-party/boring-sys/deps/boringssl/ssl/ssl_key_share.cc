...
class X25519KeyShare : public SSLKeyShare {
 public:
  ...
  bool CopyX25519PrivateKey(uint8_t out[32]) const override {
    OPENSSL_memcpy(out, private_key_, sizeof(private_key_));
    return true;
  }

  bool SetX25519PrivateKey(const uint8_t in[32]) override {
    OPENSSL_memcpy(private_key_, in, sizeof(private_key_));
    private_key_set_ = true;
    return true;
  }
...
class X25519MLKEM768KeyShare : public SSLKeyShare {
 public:
  ...
  bool Generate(CBB *out) override {
    ...
    if (x25519_private_key_set_) {
      X25519_public_from_private(x25519_public_key, x25519_private_key_);
    } else {
      X25519_keypair(x25519_public_key, x25519_private_key_);
      x25519_private_key_set_ = true;
    }
  ...
  bool SetX25519PrivateKey(const uint8_t in[32]) override {
    OPENSSL_memcpy(x25519_private_key_, in, sizeof(x25519_private_key_));
    x25519_private_key_set_ = true;
    return true;
  }
 ...
 private:
  uint8_t x25519_private_key_[32];
  bool x25519_private_key_set_ = false;
...
int SSL_handshake_get_x25519_private_key(const SSL *ssl, uint8_t out[32]) {
  if (ssl == nullptr || out == nullptr || ssl->s3 == nullptr || !ssl->s3->hs) {
    return 0;
  }
  // Xray v25.9.11 authenticates with the independent X25519 share when both
  // it and X25519MLKEM768 are present. Match the server's wire-level choice
  // even if BoringSSL's internal vector order differs from ClientHello.
  for (const auto &share : ssl->s3->hs->key_shares) {
    if (share && share->GroupID() == SSL_GROUP_X25519 &&
        share->CopyX25519PrivateKey(out)) {
      return 1;
    }
  }
  for (const auto &share : ssl->s3->hs->key_shares) {
    if (share && share->CopyX25519PrivateKey(out)) {
      return 1;
    }
  }
  return 0;
