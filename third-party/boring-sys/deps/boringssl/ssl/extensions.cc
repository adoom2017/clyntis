...
bool tls12_check_peer_sigalg(const SSL_HANDSHAKE *hs, uint8_t *out_alert,
  ...
  if ((!hs->config->allow_unadvertised_peer_sigalg &&
...
bool ssl_setup_key_shares(SSL_HANDSHAKE *hs, uint16_t override_group_id) {
  ...
  CBB key_exchange;
  uint8_t shared_x25519_private[32];
  bool have_shared_x25519_private = false;
  for (const uint16_t group_id : *selected_key_shares) {
    UniquePtr<SSLKeyShare> key_share = SSLKeyShare::Create(group_id);
    if (key_share == nullptr) {
      return false;
    }
    if (hs->config->reuse_x25519_key_share &&
        have_shared_x25519_private) {
      key_share->SetX25519PrivateKey(shared_x25519_private);
    }
    if (!CBB_add_u16(cbb.get(), group_id) ||                       //
    ...
    if (hs->config->reuse_x25519_key_share &&
        !have_shared_x25519_private &&
        key_share->CopyX25519PrivateKey(shared_x25519_private)) {
      have_shared_x25519_private = true;
    }
...
static bool ext_delegated_credential_add_clienthello(
    const SSL_HANDSHAKE *hs, CBB *out, CBB *out_compressible,
    ssl_client_hello_type_t type) {
  if (hs->config->delegated_credential_sigalgs.empty()) {
    return true;
  }
  ...
  for (uint16_t sigalg : hs->config->delegated_credential_sigalgs) {
    if (!CBB_add_u16(&algorithms, sigalg)) {
      return false;
    }
  }
  return CBB_flush(out);
...
bool ssl_setup_extension_permutation(SSL_HANDSHAKE *hs) {
  if (!hs->config->extension_order.empty()) {
    ...
    size_t out = 0;
    for (uint16_t type : hs->config->extension_order) {
      uint32_t index;
      if (tls_extension_find(&index, type) == nullptr || used[index]) {
        OPENSSL_PUT_ERROR(SSL, SSL_R_INVALID_COMMAND);
        return false;
      }
      used[index] = true;
      permutation[out++] = static_cast<uint8_t>(index);
    }
    for (size_t index = 0; index < kNumExtensions; index++) {
      if (!used[index]) {
        permutation[out++] = static_cast<uint8_t>(index);
      }
    }
...
int SSL_set1_extension_order(SSL *ssl, const uint16_t *extension_types,
  ...
  if (ssl == nullptr || !ssl->config || num_extension_types > UINT8_MAX) {
    return 0;
  }
  if (num_extension_types != 0 && extension_types == nullptr) {
    return 0;
  }
  for (size_t i = 0; i < num_extension_types; i++) {
    uint32_t ignored;
    if (bssl::tls_extension_find(&ignored, extension_types[i]) == nullptr) {
      return 0;
    }
    for (size_t j = 0; j < i; j++) {
      if (extension_types[j] == extension_types[i]) {
        return 0;
      }
    }
  }
  return ssl->config->extension_order.CopyFrom(
             Span(extension_types, num_extension_types))
             ? 1
             : 0;
}
