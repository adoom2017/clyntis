...
void SSL_set_allow_unadvertised_peer_sigalg(SSL *ssl, int enabled) {
  if (ssl->config) {
    ssl->config->allow_unadvertised_peer_sigalg = enabled != 0;
  }
}

void SSL_set_reuse_x25519_key_share(SSL *ssl, int enabled) {
  if (ssl->config) {
    ssl->config->reuse_x25519_key_share = enabled != 0;
  }
}

int SSL_set1_tls13_cipher_order(SSL *ssl, const uint16_t *cipher_ids,
                                size_t num_cipher_ids) {
  if (!ssl->config || num_cipher_ids == 0) {
    return 0;
  }
  for (uint16_t id : Span(cipher_ids, num_cipher_ids)) {
  ...
  return ssl->config->tls13_cipher_order.CopyFrom(
      Span(cipher_ids, num_cipher_ids));
}

int SSL_set1_delegated_credential_sigalgs(SSL *ssl,
                                          const uint16_t *sigalgs,
                                          size_t num_sigalgs) {
  if (!ssl->config) {
    return 0;
  }
  return ssl->config->delegated_credential_sigalgs.CopyFrom(
      Span(sigalgs, num_sigalgs));
}
