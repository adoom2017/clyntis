...
static bool ssl_write_client_cipher_list(const SSL_HANDSHAKE *hs, CBB *out,
  ...
  if (hs->max_version >= TLS1_3_VERSION) {
    ...
    const bssl::Span<const uint16_t> ciphers =
        !ssl->config->tls13_cipher_order.empty()
            ? Span<const uint16_t>(ssl->config->tls13_cipher_order)
...
bool ssl_add_client_hello(SSL_HANDSHAKE *hs) {
  ...
  if (ssl->ctx->client_hello_cb != nullptr) {
    ...
    // Keep the expected legacy_session_id echo aligned with the bytes the
    // callback put on the wire.
