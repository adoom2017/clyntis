...
// SSL_set_allow_unadvertised_peer_sigalg permits a peer CertificateVerify to
// use a signature algorithm omitted from this connection's ClientHello. The
// signature must still be compatible with the peer public key and verify
// successfully. This narrow compatibility switch is used by REALITY, whose
// authenticated ephemeral Ed25519 certificate must not alter the emulated
// browser signature_algorithms extension.
OPENSSL_EXPORT void SSL_set_allow_unadvertised_peer_sigalg(SSL *ssl,
                                                            int enabled);

// SSL_set_reuse_x25519_key_share makes X25519-based key shares in one
// ClientHello reuse the same ephemeral X25519 key. REALITY uses this so
// servers which authenticate either the classic or hybrid share derive the
// same authentication key. It is disabled by default.
OPENSSL_EXPORT void SSL_set_reuse_x25519_key_share(SSL *ssl, int enabled);
...
// REALITY hooks. The callback runs after ClientHello serialization and before
// the message is added to the transcript. It may mutate |msg| in place.
...
// SSL_set1_extension_order fixes the ClientHello extension order prefix to
// |extension_types|. Other known extensions are appended in BoringSSL's table
// order. This repository extension is used by deterministic browser profiles.
OPENSSL_EXPORT int SSL_set1_extension_order(
    SSL *ssl, const uint16_t *extension_types, size_t num_extension_types);

// SSL_set1_tls13_cipher_order sets the exact TLS 1.3 cipher suite order used
// in ClientHello. Every value must identify a supported TLS 1.3 cipher.
OPENSSL_EXPORT int SSL_set1_tls13_cipher_order(
    SSL *ssl, const uint16_t *cipher_ids, size_t num_cipher_ids);

// SSL_set1_delegated_credential_sigalgs advertises the signature algorithms
// accepted for delegated credentials (RFC 9345).
OPENSSL_EXPORT int SSL_set1_delegated_credential_sigalgs(
    SSL *ssl, const uint16_t *sigalgs, size_t num_sigalgs);

// SSL_set_record_size_limit advertises RFC 8449's record_size_limit extension.
// Passing zero disables the extension.
